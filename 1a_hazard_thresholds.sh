...
if [ -f "$file_temp_read" ]; then
	while IFS= read -r line; do
		...
		cdo -yearsel,1991/2020 $file_temp_input $file_temp_ref
...
if [ -f "$file_prec_read" ]; then
	while IFS= read -r line; do
		...
		cdo -yearsel,1991/2020 $file_prec_input $file_prec_ref
...
if [ -f "$file_wind_read" ]; then
	while IFS= read -r line; do
		...
		cdo -yearsel,1991/2020 $file_wind_input $file_wind_ref
